...
docker exec -it cli bash -c "cd /opt/gopath/src/github.com/hyperledger/fabric/chaincode/$CHAINCODE_NAME && go mod init $CHAINCODE_NAME && go mod tidy"

# 패키징
docker exec -it cli bash -c "peer lifecycle chaincode package $PACKAGE_NAME --path /opt/gopath/src/github.com/hyperledger/fabric/chaincode/$CHAINCODE_NAME --lang golang --label $CHAINCODE_LABEL"
...
for org in 1 2 3; do
  for peer in 0 1; do
    ...
    docker exec -it cli bash -c "CORE_PEER_ADDRESS=$PEER_ADDRESS \
...
# 패키징된 ID 확인
PACKAGE_ID=$(docker exec -it cli bash -c "peer lifecycle chaincode queryinstalled" | grep $CHAINCODE_LABEL | sed -n 's/^Package ID: \(\S*\), Label:.*$/\1/p')
echo "Package ID: $PACKAGE_ID"

# 모든 Org에서 승인
for org in 1 2 3; do
  echo "Org${org} 체인코드 승인 중"
  docker exec -it cli bash -c "CORE_PEER_ADDRESS=peer0.org${org}.data-collector.com:${peer_ports[peer0.org${org}.data-collector.com]} \
    CORE_PEER_LOCALMSPID=Org${org}MSP \
...
# 커밋
echo "커밋 중"
docker exec -it cli bash -c "peer lifecycle chaincode commit -o orderer.data-collector.com:7050 --ordererTLSHostnameOverride orderer.data-collector.com --tls --cafile /etc/hyperledger/orderer/tls/ca.crt --channelID mychannel --name $CHAINCODE_NAME --version 1.0 --sequence 1 --peerAddresses peer0.org1.data-collector.com:7051 --tlsRootCertFiles /etc/hyperledger/fabric/crypto-config/peerOrganizations/org1.data-collector.com/peers/peer0.org1.data-collector.com/tls/ca.crt --peerAddresses peer0.org2.data-collector.com:9051 --tlsRootCertFiles /etc/hyperledger/fabric/crypto-config/peerOrganizations/org2.data-collector.com/peers/peer0.org2.data-collector.com/tls/ca.crt --peerAddresses peer0.org3.data-collector.com:11051 --tlsRootCertFiles /etc/hyperledger/fabric/crypto-config/peerOrganizations/org3.data-collector.com/peers/peer0.org3.data-collector.com/tls/ca.crt"

# 커밋 제대로 됐는지 쿼리 <-- 나중에 빼기
echo "커밋 결과:"
docker exec -it cli bash -c "peer lifecycle chaincode querycommitted --channelID mychannel --name $CHAINCODE_NAME"
